...
Phase 4 of trntensor introduced `ShardedTensor` and output-parallel einsum dispatch in [v0.9.0](https://trnsci.dev/blog/trntensor-v080v090-dispatch-owns-routing-and-placement/). [v0.10.0](https://trnsci.dev/blog/trntensor-v0100-the-test-surface-names-the-interface/) lifted the reduce-parallel gate on CPU, replacing `NotImplementedError` with `_mock_allreduce` and proving the correctness contract without hardware. [v0.11.0](https://trnsci.dev/blog/trntensor-v0110-stochastic-rounding-at-the-psumsbuf-boundary/) added `precision="sr"` at the PSUM→SBUF boundary.
